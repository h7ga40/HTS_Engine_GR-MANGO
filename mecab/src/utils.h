...
void inline dtoa(float val, char *s) {
...
inline float logsumexp(float x, float y, bool flg) {
#define MINUS_LOG_EPSILON  50

  if (flg) return y;  // init mode
  float vmin = std::min<float>(x, y);
  float vmax = std::max<float>(x, y);
...
inline short int tocost(float d, int n) {
  static const short max = +32767;
  static const short min = -32767;
  return static_cast<short>(std::max<float>(
                                std::min<float>(
                                    -n * d,
