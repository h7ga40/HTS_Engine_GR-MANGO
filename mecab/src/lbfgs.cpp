...
namespace {
static const float ftol = 1e-4;
static const float xtol = 1e-16;
static const float eps  = 1e-7;
static const float lb3_1_gtol = 0.9;
static const float lb3_1_stpmin = 1e-20;
static const float lb3_1_stpmax = 1e20;
...
inline float pi(float x, float y) {
...
inline float ddot_(int size, const float *dx, const float *dy) {
  return std::inner_product(dx, dx + size, dy, 0.0);
}

void mcstep(float *stx, float *fx, float *dx,
            float *sty, float *fy, float *dy,
            float *stp, float fp, float dp,
            int *brackt,
            float stpmin, float stpmax,
            int *info) {
  bool bound = true;
...
class LBFGS::Mcsrch {
 ...
 public:
  ...
  void mcsrch(int size,
              float *x,
              float f, const float *g, float *s,
              float *stp,
              int *info, int *nfev, float *wa, bool orthant, float C) {
    const float p5 = 0.5;
    const float p66 = 0.66;
    const float xtrapf = 4.0;
    const int maxfev = 20;
...
void LBFGS::lbfgs_optimize(int size,
 ...
 L172:
    ...
    float xnorm = std::max(1.0f, std::sqrt(ddot_(size, &x[1], &x[1])));
    if (gnorm / xnorm <= eps) {
