...
void FeatureIndex::set_alpha(const float *alpha) {
  alpha_ = alpha;
}
...
bool DecoderFeatureIndex::openFromArray(const char *begin, const char *end) {
  ...
  const size_t expected_file_size =
      (sizeof(float) + sizeof(uint64_t)) * maxid_ + sizeof(maxid) + 32;
  if (expected_file_size != file_size) {
    return false;
  }
  charset_ = ptr;
  ptr += 32;
  alpha_ = reinterpret_cast<const float *>(ptr);
...
void EncoderFeatureIndex::shrink(size_t freq,
  ...
  std::vector<float> observed_new(maxid_);
...
bool FeatureIndex::convert(const Param &param,
  ...
  scoped_fixed_array<char, BUF_SIZE> buf;
  char *column[4];
  std::vector<std::pair<uint64_t, float> > dic;
  ...
  while (ifs.getline(buf.get(), buf.size())) {
    ...
    const float alpha = atof(column[0]);
    dic.push_back(std::pair<uint64_t, float>(fp, alpha));
  }
  ...
  for (size_t i = 0; i < dic.size(); ++i) {
    const float alpha = dic[i].second;
