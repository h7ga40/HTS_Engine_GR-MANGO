...
class CRFLearner {
 public:
  static int run(Param *param) {
    ...
    const float C = param->get<float>("cost");
    const float eta = param->get<float>("eta");
    ...
    for (size_t itr = 0; ;  ++itr) {
      ...
      const float p = 1.0 * micro_c / micro_p;
      const float r = 1.0 * micro_c / micro_r;
      const float micro_f = 2 * p * r / (p + r);

      for (size_t i = 0; i < psize; ++i) {
        const float penalty = (alpha[i] - old_alpha[i]);
        obj += (penalty * penalty / (2.0 * C));
        expected[i] = expected[i] - observed[i] + penalty / C;
      }

      const float diff = (itr == 0 ? 1.0 :
